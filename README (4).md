...
An end-to-end data analytics project that mirrors the real workflow of a professional Data Analyst — from raw data to business decisions. It explores 3,900 retail transactions to uncover customer segments, purchase drivers, and revenue trends, and turns those insights into an interactive Power BI dashboard, a written report, and concrete business recommendations.
...
To answer it, the project was broken into four connected stages, each handled with the tool best suited to the job:

✅ **Data Preparation, Modeling & Exploratory Data Analysis (Python)** — clean and transform the raw dataset, handle missing values, and engineer features that make later analysis possible.

✅ **Data Analysis (SQL)** — load the cleaned data into PostgreSQL, simulate business transactions, and run structured queries to extract insights on customer segments, loyalty, and purchase drivers.

✅ **Visualization & Insights (Power BI)** — build an interactive dashboard that highlights key patterns and trends, so stakeholders can make data-driven decisions themselves.

✅ **Report and Presentation** — summarize key findings and business recommendations in a written report and a stakeholder-facing presentation deck.

The result is a project that mirrors how a Data Analyst / Data Engineer actually works in industry: moving data through Python → SQL → BI → business communication, rather than treating each as an isolated exercise.
...
4. **Open `notebooks/Customer_Shopping_Behavior_Analysis.ipynb`**
...
5. **Load the data from the notebook into PostgreSQL (or MySQL / SQL Server)**
   - Create a local database
   - Copy `.env.example` to `.env` and fill in your own database credentials
     ```bash
     cp .env.example .env
     ```
   - Run the notebook cells to load the cleaned data into your database

6. **Run the business-question queries**
   - Open [`sql/customer_behavior_sql_queries.sql`](sql/customer_behavior_sql_queries.sql)
   - Execute the queries against your loaded `customer` table

7. **Explore the dashboard**
   - Open [`dashboard/customer_behavior_dashboard.pbix`](dashboard/customer_behavior_dashboard.pbix) in Power BI Desktop
   - Interact with the slicers to explore different customer segments

8. **Review the report and presentation**
   - Full write-up: [`reports/Customer_Shopping_Behavior_Analysis.pdf`](reports/Customer_Shopping_Behavior_Analysis.pdf)
   - Stakeholder deck: [`presentation/Customer-Shopping-Behavior-Analysis.pptx`](presentation/Customer-Shopping-Behavior-Analysis.pptx)
...
MIT — feel free to fork, star, and reference this project.

---

## 👨‍💻 About the Author

Hey, I'm **Ganesh Sargar**, a final-year B.Tech Computer Science and Engineering student specializing in IoT, Cyber Security, and Blockchain Technology at Shivaji University (ADCET, Ashta). I'm building hands-on projects in Data Analytics, SQL, and Power BI while preparing for Data Analyst and Data Engineer roles.
